...
public static class LogEnricher
{
    ...
    public static void EnrichFromRequest(IDiagnosticContext diagnosticContext, HttpContext httpContext)
    {
        ...
        if (endpoint is not null) diagnosticContext.Set("EndpointName", endpoint.DisplayName);
    }

    /// <summary>
    ///     Shows request logs just in Information log level, this means we should set serilog `MinimumLevel:Default` to
    ///     `Information` (for example ir doesn't show in Warning level) and shows health check logs just in `Debug` log level
    ///     that is higher than Information (for getting fewer logs in the output).
    /// </summary>
    ...
    {
        var isSwagger = ctx.Request.Path.Value?.Contains("swagger", StringComparison.Ordinal) ?? false;
