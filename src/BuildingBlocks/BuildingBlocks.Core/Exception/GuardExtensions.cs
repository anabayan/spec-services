...
public static partial class GuardExtensions
{
    ...
    {
        if (string.IsNullOrEmpty(input)) throw exception;
    ...
    {
        if (string.IsNullOrWhiteSpace(input)) throw exception;
    ...
    {
        if (input.CompareTo(default(T)) <= 0) throw exception;
    ...
    {
        if (input == false) throw exception;
    ...
    {
        if (date == default) throw new InvalidDateException(date);
    ...
    {
        if (string.IsNullOrWhiteSpace(email)) throw exception;

        if (email.Length > 100) throw exception;

        email = email.ToLowerInvariant();
        if (!_regex.IsMatch(email)) throw exception;
    ...
    {
        if (string.IsNullOrWhiteSpace(currency) || currency.Length != 3) throw exception;

        currency = currency.ToUpperInvariant();
        if (!_allowedCurrency.Contains(currency)) throw exception;
    ...
    {
        if (string.IsNullOrWhiteSpace(phoneNumber)) throw exception;

        if (phoneNumber.Length < 7) throw exception;

        if (phoneNumber.Length > 15) throw exception;
