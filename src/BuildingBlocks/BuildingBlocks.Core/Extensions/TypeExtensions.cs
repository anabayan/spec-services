...
public static class TypeExtensions
{
    ...
    public static MethodInfo? GetGenericMethod(
        ...
        Type? returnType = null)
    {
        var res = (from m in t.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)
                where m.Name == name &&
    ...
    public static Task<dynamic>? InvokeGenericMethodAsync(
        ...
        params object[] parameters)
    {
        var awaitable = InvokeGenericMethod(type, methodName, genericTypes, returnType, parameters);
    ...
    public static MethodInfo[] GetExtensionMethods(this Type t)
    {
        ...
        foreach (var item in AppDomain.CurrentDomain.GetAssemblies()) AssTypes.AddRange(item.GetTypes());
    ...
    public static bool IsDerivativeOf(this Type type, Type typeToCompare)
    {
        ...
        if (retVal) retVal = type.BaseType == typeToCompare;

        if (!retVal && type.BaseType != null) retVal = type.BaseType.IsDerivativeOf(typeToCompare);
    ...
    public static Type UnwrapEnumType(this Type type)
    {
        ...
        if (!underlyingNonNullableType.GetTypeInfo().IsEnum) return type;
        ...
        bool addIfAlreadyExists)
    {
        foreach (var openInterface in openMessageInterfaces)
        {
            ...
            foreach (var @interface in interfaces.Distinct())
            {
                ...
                {
                    if (matches.Count() > 1) matches.RemoveAll(m => !IsMatchingWithInterface(m, @interface));
